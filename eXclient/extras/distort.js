...
function initNonlinearTransform(vs, fs, uniformNames, uniformLambdas){
    if(shared.distortionsActive === true) unInitNonlinearTransform();
    if(isTainted(window.ctx) === true) shared.unTaintCanvas();
    storedDrawImg = ctx.drawImage;
    ctx.drawImage = () => {};
    shared.distortionsActive = true;
    ...
    shared.resizeWebGl();
    shared.resizeFns.push(shared.resizeWebGl);
...
function unInitNonlinearTransform(){
    if(shared.distortionsActive === false) return;
